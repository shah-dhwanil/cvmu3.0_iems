...
async def get_user(request, user_id: UUID, **_):
    if request.ctx.user.role  == RoleEnum.STUDENT and request.ctx.user.user_id != user_id:
...
async def update_user_password(
    ...
    if request.ctx.user.role in [RoleEnum.STUDENT,RoleEnum.TEACHER] and request.ctx.user.user_id != user_id:
        return JSONResponse(AccessDenied().model_dump_json(), 403)
